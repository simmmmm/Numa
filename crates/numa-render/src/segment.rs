...
pub fn named_something(alpha: &Alpha) -> bool {
    alpha.data.iter().map(|value| *value as f64).sum::<f64>() / alpha.data.len().max(1) as f64 > 0.001
}
...
mod tests {
    ...
    fn another_segmenter_on_a_frame() {
        ...
        if let Ok(dir) = std::env::var("GPU") {
            numa_infer::enable_gpu(std::path::Path::new(&dir), &std::env::temp_dir().join("numa-gpu-attempt"));
        }
    ...
    fn where_a_click_lands() {
        ...
        let Some(embedding) = crate::sam::encode(&frame) else { return };
