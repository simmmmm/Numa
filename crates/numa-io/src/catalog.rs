...
pub struct Glance {
    ...
    pub covers: Vec<(PathBuf, i64, Option<String>)>,
}

pub const COVERS: usize = 48;
...
impl FileType {
    fn admits(self, path: &Path) -> bool {
        match self {
            FileType::Any => true,
            FileType::Raw => crate::raw::is_raw(path),
            FileType::NotRaw => !crate::raw::is_raw(path),
        }
    }
}
...
impl Sort {
    fn order_by(self, reversed: bool) -> &'static str {
        match (self, reversed) {

            (Sort::Captured, false) => "COALESCE(taken, mtime) ASC, path ASC",
            (Sort::Captured, true) => "COALESCE(taken, mtime) DESC, path ASC",
            (Sort::Name, false) => "path ASC",
            (Sort::Name, true) => "path DESC",
            (Sort::Rating, false) => "rating DESC, COALESCE(taken, mtime) ASC",
            (Sort::Rating, true) => "rating ASC, COALESCE(taken, mtime) ASC",
            (Sort::Sharpness, false) => "sharpness DESC NULLS LAST, COALESCE(taken, mtime) ASC",
            (Sort::Sharpness, true) => "sharpness ASC NULLS LAST, COALESCE(taken, mtime) ASC",
            (Sort::Suggested, false) => "suggested DESC NULLS LAST, COALESCE(taken, mtime) ASC",
            (Sort::Suggested, true) => "suggested ASC NULLS LAST, COALESCE(taken, mtime) ASC",
        }
    }

    fn compare(self, reversed: bool, a: &Photo, b: &Photo) -> std::cmp::Ordering {
        ...
        let (at_a, at_b) = (when(a), when(b));
        let captured = (at_a, &a.path).cmp(&(at_b, &b.path));
        let order = match self {
            Sort::Captured => captured,
            Sort::Name => a.path.cmp(&b.path),
            Sort::Rating => b.rating.cmp(&a.rating).then(at_a.cmp(&at_b)),
            Sort::Sharpness => best(a.sharpness, b.sharpness).then(at_a.cmp(&at_b)),
            Sort::Suggested => best(a.suggested, b.suggested).then(at_a.cmp(&at_b)),
        };
        match reversed {
            true => order.reverse(),
            false => order,
...
pub fn library_of(photo_id: i64) -> i64 {
    split_id(photo_id).0
}
...
pub fn default_path() -> PathBuf {
    numa_core::paths::data_dir().join("catalog.db")
}
...
impl Catalog {
    ...
    pub fn unanalysed(&self, library_id: i64, version: i64) -> Result<Vec<Photo>, String> {
        ...
        let mut stmt = open
            .conn
            .prepare(
                "SELECT p.id, p.path, p.mtime, p.rating, p.flag, p.taken \
    ...
    pub fn descriptions(&self, photo_ids: &[i64]) -> Result<HashMap<i64, crate::export::Description>, String> {
        let albums: HashMap<String, String> = self.albums()?.into_iter().collect();
        let mut everyone: HashMap<i64, Vec<(String, Vec<i64>)>> = HashMap::new();
        let mut found = HashMap::new();
        for &id in photo_ids {
            let (open, local) = self.photo(id)?;
            let rating: u8 = open
                .conn
                .query_row("SELECT rating FROM photos WHERE id = ?1", [local], |row| row.get(0))
                .optional()
                .map_err(text)?
                .unwrap_or(0);
            ...
            if !everyone.contains_key(&library) {
                everyone.insert(library, self.people(library)?);
            }
    ...
    pub fn scale(&self, library_id: i64) -> Result<numa_cull::Scale, String> {
        let open = self.library(library_id)?;
        let mut stmt = open
            .conn
            .prepare("SELECT COALESCE(face_sharpness, sharpness) FROM analysis WHERE sharpness IS NOT NULL")
            .map_err(text)?;
        let values = stmt
            ...
            .map_err(text)?;
        Ok(numa_cull::Scale::of(values))
    }

    pub fn analysed(&self, library_id: i64) -> Result<Vec<(i64, numa_cull::Frame, Option<f32>)>, String> {
        let open = self.library(library_id)?;
        let mut stmt = open
            .conn
            .prepare(
    ...
    pub fn paths(&self, library_id: i64) -> Result<Vec<PathBuf>, String> {
        let open = self.library(library_id)?;
        let mut statement = open.conn.prepare("SELECT path FROM photos").map_err(text)?;
        let paths = statement
            .query_map([], |row| Ok(open.absolute(&row.get::<_, String>(0)?)))
            .map_err(text)?
            .collect::<Result<_, _>>()
            .map_err(text)?;
        Ok(paths)
    ...
    pub fn glance(&self, library_id: i64, album: Option<&str>, only: Option<&[i64]>) -> Result<Glance, String> {
        ...
        let mut narrow =
            String::from(" AND (?1 IS NULL OR id IN (SELECT photo_id FROM album_photos WHERE album = ?1))");
        if let Some(only) = only {

            let mine: Vec<String> =
                only.iter().filter(|id| split_id(**id).0 == library_id).map(|id| split_id(*id).1.to_string()).collect();

            match mine.is_empty() {
                true => narrow.push_str(" AND 0"),
                false => narrow.push_str(&format!(" AND id IN ({})", mine.join(","))),
            }
        }

        let when = "COALESCE(taken, mtime)";
        let (photos, first, last) = open
            .conn
            .query_row(
                &format!("SELECT COUNT(*), MIN({when}), MAX({when}) FROM photos WHERE 1{narrow}"),
                params![album],
        ...
        let mut statement = open
            ...
            .prepare(&format!(
                "SELECT path, mtime, edits FROM ( \
                 ...
                 WHERE nth = 1 ORDER BY {best} LIMIT {COVERS}",
                best = "rating DESC, flag DESC, edits IS NOT NULL DESC, suggested DESC NULLS LAST, COALESCE(taken, mtime) DESC",
            ))
            .map_err(text)?;
    ...
    pub fn relocate_library(&self, id: i64, to: &Path) -> Result<(), String> {
        if !to.is_dir() {
            return Err(format!("{} is not a folder", to.display()));
        }

        let taken: Option<i64> = self
            .home
            .query_row(
                "SELECT id FROM libraries WHERE path = ?1 AND id <> ?2",
                params![to.to_string_lossy(), id],
                |row| row.get(0),
            )
            .optional()
            .map_err(text)?;
        if taken.is_some() {
            return Err(format!("{} is already in the catalog", to.display()));
        }
        self.opened.borrow_mut().remove(&id);
        self.home
            .execute("UPDATE libraries SET path = ?1 WHERE id = ?2", params![to.to_string_lossy(), id])
            .map_err(text)?;
        Ok(())
    }
    ...
    pub fn photos_everywhere(&self, filter: &Filter) -> Result<Vec<Photo>, String> {
        ...
        photos.sort_by(|a, b| filter.sort.compare(filter.reversed, a, b));
        Ok(photos)
    }
    ...
    pub fn photos(&self, library_id: i64, filter: &Filter) -> Result<Vec<Photo>, String> {
        ...
        let best = "a.burst IS NOT NULL \
                    ...
                    AND COALESCE(a.face_sharpness, a.sharpness) \
                        = MAX(COALESCE(a.face_sharpness, a.sharpness)) OVER (PARTITION BY a.burst)";
        ...
        sql.push_str(filter.sort.order_by(filter.reversed));
...
mod tests {
    ...
    #[test]
    fn a_librarys_covers_are_its_best_and_never_the_rejected() {
        let root = temp_dir("covers");
        for name in ["a.RAF", "b.RAF", "c.RAF", "d.RAF", "e.RAF"] {
        ...
        catalog.set_rating(id("c.RAF"), 3).unwrap();
        catalog.set_flag(id("e.RAF"), Flag::Picked).unwrap();
        let mut edited = Document::new(root.join("a.RAF").to_string_lossy().into_owned());
        edited.set_basic(Basic::with(|b| b.tone.exposure = 0.5));
        catalog.save_edits(id("a.RAF"), &edited).unwrap();
        catalog.set_rating(id("d.RAF"), 5).unwrap();
        catalog.set_flag(id("d.RAF"), Flag::Rejected).unwrap();

        let covers: Vec<String> = catalog
            .glance(library.id, None, None)
            .unwrap()
            .covers
            .iter()
    ...
    fn a_mask_survives_the_catalog_with_everything_on_it() {
        use numa_core::mask::{Mask, RegionPoint, Shape, Stroke};
        ...
        mask.basic = Basic::with(|b| b.tone.exposure = -1.5);
        mask.inverted = true;
        ...
        assert_eq!(back[0].basic.tone.exposure, -1.5);
    ...
    fn a_person_is_every_photograph_with_a_face_like_theirs() {
        ...
        let frame = numa_cull::Frame { sharpness: 1.0, blown: 0.0, hash: 0, ..Default::default() };
    ...
    fn analysis_filters_the_grid_without_touching_the_stars() {
        ...
        for (photo, frame) in all.iter().zip(frames.iter()) {
            ...
            catalog.save_analysis(photo.id, numa_cull::VERSION, frame, None, None).unwrap();
        }
        assert!(catalog.unanalysed(library.id, numa_cull::VERSION).unwrap().is_empty());
    ...
    fn best_of_burst_is_computed_and_cannot_go_stale() {
        ...
        let frames = [
            numa_cull::Frame { sharpness: 0.9, blown: 0.0, hash: 1, ..Default::default() },
            numa_cull::Frame { sharpness: 0.3, blown: 0.0, hash: 1, ..Default::default() },
            numa_cull::Frame { sharpness: 0.7, blown: 0.0, hash: 2, ..Default::default() },
        ];
        for (photo, frame) in all.iter().zip(frames.iter()) {
            catalog.save_analysis(photo.id, numa_cull::VERSION, frame, None, None).unwrap();
        }
...
"#;
    ...
    fn a_single_catalog_is_split_into_the_libraries_folders() {
        ...
        let path = home.join("catalog.db");
        {
            ...
            let edits = serde_json::to_string(&{
                ...
                doc.set_basic(Basic::with(|b| b.tone.exposure = 0.7));
                doc
            })
            .unwrap();
            conn.execute(
        ...
        assert_eq!(catalog.load_edits(a.id).unwrap().unwrap().basic().tone.exposure, 0.7);
    ...
    fn rates_flags_and_filters() {
        ...
        doc.set_basic(Basic::with(|b| b.tone.exposure = 0.7));
        catalog.save_edits(raf.id, &doc).unwrap();
        assert_eq!(catalog.load_edits(raf.id).unwrap().unwrap().basic().tone.exposure, 0.7);
    ...
    #[test]
    fn a_photograph_says_which_library_it_came_from() {
        let (one, two) = (temp_dir("trip-one"), temp_dir("trip-two"));
        for root in [&one, &two] {
            std::fs::write(root.join("DSCF2310.RAF"), b"x").unwrap();
        }

        let catalog = Catalog::in_memory().unwrap();
        let japan = catalog.add_library(&one).unwrap();
        let italy = catalog.add_library(&two).unwrap();
        for library in [&japan, &italy] {
            catalog.sync_library(library).unwrap();
        }

        let from = |library: &Library| catalog.photos(library.id, &Filter::default()).unwrap().remove(0);
        let (a, b) = (from(&japan), from(&italy));
        assert_eq!(a.path.file_name(), b.path.file_name(), "the same name in both, which is the point");
        assert_ne!(a.id, b.id, "two catalogs each counting from one must not hand out one id twice");
    ...
    #[test]
    fn the_label_and_the_pick_agree() {
        ...
        let photos = catalog.photos(library.id, &Filter::default()).unwrap();
        let frames = [
            numa_cull::Frame { sharpness: 1.1, blown: 0.0, hash: 0b01, ..Default::default() },
            numa_cull::Frame { sharpness: 0.4, blown: 0.0, hash: 0b01, ..Default::default() },
        ];
        let faces = [Some(0.2f32), Some(1.0f32)];
        for ((photo, frame), face) in photos.iter().zip(frames.iter()).zip(faces.iter()) {
            catalog.save_analysis(photo.id, numa_cull::VERSION, frame, Some(1), *face).unwrap();
        }
        ...
        let labelled = catalog
            .photos(library.id, &Filter { best_of_burst: true, ..Default::default() })
            .unwrap();
        assert_eq!(labelled.len(), 1, "one of the two is the best of the run");
        assert!(
            labelled[0].path.ends_with("b.RAF"),
            "the card labelled the frame with the crisp background: {:?}",
            labelled[0].path
        );
    ...
    fn the_order_is_the_shutter_and_not_the_copy() {
        ...
        for (name, mtime, taken) in [("a.RAF", 300, Some(10)), ("b.RAF", 200, Some(20)), ("c.RAF", 100, None)] {
            let touched = open.conn
                .execute("UPDATE photos SET mtime = ?1, taken = ?2 WHERE path = ?3", params![mtime, taken, name])
                .unwrap();
            assert_eq!(touched, 1, "no row for {name}: paths are stored relative to the library");
        }

        let order = |sort: Sort| {
            catalog
                .photos(library.id, &Filter { sort, ..Filter::default() })
                .unwrap()
                .iter()
                .map(|photo| photo.path.file_name().unwrap().to_string_lossy().to_string())
                .collect::<Vec<_>>()
        ...
        assert_eq!(order(Sort::Captured), ["a.RAF", "b.RAF", "c.RAF"], "the copy's order won");

        assert_eq!(order(Sort::Rating), ["a.RAF", "b.RAF", "c.RAF"]);

        let photos = catalog.photos(library.id, &Filter::default()).unwrap();
        let mut across = photos.clone();
        across.sort_by(|a, b| Sort::Captured.compare(false, a, b));
        assert_eq!(
            across.iter().map(|p| p.path.file_name().unwrap()).collect::<Vec<_>>(),
    ...
    fn photographs_from_several_libraries_sort_as_one_query_would() {
        ...
        let order = |sort: Sort| {
            ...
            sorted.sort_by(|a, b| sort.compare(false, a, b));
