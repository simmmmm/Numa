...
mod tests {
    ...
    #[test]
    fn a_display_profile_turns_srgb_into_the_screens_own_numbers() {
        use numa_render::display::Display;
        for screen in [ColourSpace::DisplayP3, ColourSpace::AdobeRgb] {
            let display = Display::from_icc(&profile(screen)).unwrap().expect("a wide screen is not sRGB");
            let matrix = ColourSpace::Srgb.convert_to(screen).unwrap();
            for patch in [[255u8, 0, 0], [0, 255, 0], [0, 0, 255], [200, 120, 60], [128, 128, 128], [30, 200, 220], [0, 0, 0], [255, 255, 255]] {
                let linear = patch.map(|code| ColourSpace::Srgb.decode(code as f32 / 255.0));
                let expected: [u8; 3] = std::array::from_fn(|channel| {
                    let row = matrix[channel];
                    let value = row[0] * linear[0] + row[1] * linear[1] + row[2] * linear[2];
                    (screen.encode(value) * 255.0).round() as u8
                });
                let got = display.pixel(patch);
                for channel in 0..3 {
                    assert!(got[channel].abs_diff(expected[channel]) <= 1, "{screen:?} {patch:?}: {got:?} against {expected:?}");
                }
            }
            assert_eq!(display.name, format!("Numa {}", screen.name()));
        }
        assert!(Display::from_icc(&profile(ColourSpace::Srgb)).unwrap().is_none(), "an sRGB screen converts nothing");
    }
}
