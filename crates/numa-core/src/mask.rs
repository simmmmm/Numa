...
impl Stroke {
    ...
    pub fn soft_lasso(radius: f32, feather: f32, erase: bool) -> Self {
        Self { radius, feather, ..Self::lasso(erase) }
    }

    pub fn draw(&self, alpha: &mut impl Coverage) {
        if self.fill {
    ...
    fn fill_outline(&self, alpha: &mut impl Coverage) {
        ...
        let band = self.radius.max(0.0) * self.feather.clamp(0.0, 1.0) * width.max(height) as f32;
        match (band / 2.0).round() as usize {
            0 => fill_polygon(&corners, self.erase, alpha),
            reach => self.fill_soft(&corners, reach, alpha),
        }
    }

    fn fill_soft(&self, corners: &[[f32; 2]], reach: usize, alpha: &mut impl Coverage) {
        let (width, height) = (alpha.width(), alpha.height());

        let pad = reach as f32 + 1.0;
        let span = |axis: usize, size: usize| {
            let low = corners.iter().map(|p| p[axis]).fold(f32::MAX, f32::min);
            let high = corners.iter().map(|p| p[axis]).fold(f32::MIN, f32::max);
            let clip = |at: f32| at.clamp(0.0, size as f32) as usize;
            (clip((low - pad).floor()), clip((high + pad).ceil()))
        };
        let ((left, right), (top, bottom)) = (span(0, width), span(1, height));
        ...
        let shifted: Vec<[f32; 2]> =
            corners.iter().map(|p| [p[0] - left as f32, p[1] - top as f32]).collect();
        let mut hard = Alpha::new(w, h, vec![0.0; w * h]);
        fill_polygon(&shifted, false, &mut hard);

        let mut plane = Plane::new(w, h, hard.data);
        for pass in [reach - reach / 2, reach / 2] {
            if pass > 0 {
                plane = blur(&plane, pass);
            }
        }
        ...
        for y in 0..h {
            for x in 0..w {
                ...
                let covered = match plane.data[y * w + x] {
                    c if c < 1e-5 => continue,
                    c if c > 1.0 - 1e-5 => 1.0,
                    c => c,
                };
                lay(alpha, (top + y) * width + left + x, covered, self.erase);
            }
        }
    }

    pub fn draw_segment(&self, alpha: &mut impl Coverage, from: [f32; 2], to: [f32; 2]) {
        ...
        for y in top..=bottom {
            for x in left..=right {
                ...
                lay(alpha, y * alpha.width() + x, coverage, self.erase);
            }
        }
    }
}

fn fill_polygon(corners: &[[f32; 2]], erase: bool, alpha: &mut impl Coverage) {
    let (width, height) = (alpha.width(), alpha.height());
    ...
    for y in top..=bottom {
        ...
        for x in 0..width {
            ...
            lay(alpha, y * width + x, covered, erase);
        }
    }
}

fn lay(alpha: &mut impl Coverage, index: usize, covered: f32, erase: bool) {
    let was = alpha.at(index);
    alpha.set(
        index,
        match erase {
            true => was * (1.0 - covered),

            false => was + (1.0 - was) * covered,
        },
    );
}
...
impl Coverage for Stored {
    ...
    fn at(&self, index: usize) -> f32 {
        Stored::at(self, index)
    }
    fn set(&mut self, index: usize, value: f32) {
        Stored::set(self, index, value)
    }
}
...
impl Stored {
    ...
    pub fn at(&self, index: usize) -> f32 {
        self.data[index] as f32 * (1.0 / Self::STEPS)
    }

    pub fn set(&mut self, index: usize, value: f32) {
        self.data[index] = (value.clamp(0.0, 1.0) * Self::STEPS + 0.5) as u16;
    }

    pub fn values(&self) -> impl Iterator<Item = f32> + '_ {
        self.data.iter().map(|value| *value as f32 * (1.0 / Self::STEPS))
    }

    pub fn to_alpha(&self) -> Alpha {
        Alpha::new(self.width, self.height, self.data.par_iter().map(|v| *v as f32 * (1.0 / Self::STEPS)).collect())
    }
    ...
    pub fn sample(&self, u: f32, v: f32) -> f32 {
        ...
        let at = |x: usize, y: usize| self.data[y * self.width + x] as f32;
        let top = at(x0, y0) + (at(x1, y0) - at(x0, y0)) * fx;
        let bottom = at(x0, y1) + (at(x1, y1) - at(x0, y1)) * fx;
        (top + (bottom - top) * fy) * (1.0 / Self::STEPS)
    }
}

#[derive(Debug, Clone, Default)]
pub struct Pixels(pub Option<Arc<Stored>>);

impl Pixels {

    pub fn of(alpha: &Alpha) -> Self {
        Self(Some(Arc::new(Stored::new(alpha))))
    }
}
...
impl Mask {
    ...
    pub fn reshape_edge_draft(&mut self, width: usize, height: usize) -> bool {
        ...
        let mut data = vec![0.0f32; half_w * half_h];
        data.par_chunks_mut(half_w).enumerate().for_each(|(y, row)| {
            for (x, cell) in row.iter_mut().enumerate() {
                let (sx, sy) = (x * 2, y * 2);
                let at = |dx: usize, dy: usize| {
                    unshaped.at((sy + dy).min(height - 1) * width + (sx + dx).min(width - 1))
                };
                *cell = (at(0, 0) + at(1, 0) + at(0, 1) + at(1, 1)) * 0.25;
            }
        });
        let mut alpha = Alpha::new(half_w, half_h, data);
        self.shape_the_edge(&mut alpha);
        self.map = Pixels::of(&alpha);
        true
    }

    pub fn edge_is_draft(&self, width: usize, height: usize) -> bool {
        self.map.0.as_ref().is_some_and(|map| map.width != width || map.height != height)
            && self.unshaped.0.as_ref().is_some_and(|un| un.width == width && un.height == height)
    }
    ...
    pub fn reshape_edge(&mut self, width: usize, height: usize) -> bool {
        ...
        let mut alpha = unshaped.to_alpha();
        self.shape_the_edge(&mut alpha);
        self.map = Pixels::of(&alpha);
        true
    }
    ...
    pub fn shape_the_edge(&self, alpha: &mut Alpha) {
        ...
        shape_edge(alpha, self.feather, self.shift - searched, self.matted);
    }

    pub fn edge_to_search_from(&self, alpha: &mut Alpha) {

        shape_edge(alpha, 0.0, self.matte_edge, false);
    }
}

fn shape_edge(alpha: &mut Alpha, feather_setting: f32, shift_setting: f32, matted: bool) {
    ...
    let steepness = match matted {
        true => 1.0,
        false => (24.0 / (1.0 + feather_setting.clamp(0.0, 100.0) * 0.45)).max(1.0),
    };
...
impl Mask {
    ...
    pub fn field_through(&self, width: usize, height: usize, region: [f32; 4], map: [f32; 6]) -> Vec<f32> {
...
mod tests {
    ...
    fn a_square_traces_to_one_closed_path() {
        ...
        let paths = outline(&Stored::new(&alpha), 64);
    ...
    fn two_shapes_trace_to_two_paths() {
        ...
        assert_eq!(outline(&Stored::new(&alpha), 64).len(), 2);
    }
    ...
    fn nothing_selected_has_no_outline() {
        assert!(outline(&Stored::new(&Alpha::new(32, 32, vec![0.0; 32 * 32])), 32).is_empty());
        assert!(outline(&Stored::new(&Alpha::new(32, 32, vec![1.0; 32 * 32])), 32).is_empty());
    }
    ...
    fn a_found_mask_samples_its_pixels_and_covers_nothing_until_it_has_any() {
        ...
        mask.map = Pixels::of(&Alpha::new(2, 1, vec![0.0, 1.0]));
        assert!(!mask.is_pending());
    ...
    fn fill_outline_as_it_was(stroke: &Stroke, alpha: &mut impl Coverage) {
        if stroke.points.len() < 3 || alpha.width() == 0 || alpha.height() == 0 {
            return;
        }
        let (width, height) = (alpha.width(), alpha.height());
        let corners: Vec<[f32; 2]> = stroke
            .points
            .iter()
            .map(|p| [p[0] * width as f32, p[1] * height as f32])
            .collect();
    ...
    #[test]
    fn a_lasso_without_softness_fills_as_it_always_did() {

        let star: Vec<[f32; 2]> = (0..40)
            .map(|i| {
                let angle = i as f32 * std::f32::consts::TAU / 40.0;
                let reach = if i % 2 == 0 { 0.45 } else { 0.2 };
                [0.5 + reach * angle.cos(), 0.5 + reach * 1.3 * angle.sin()]
            })
            .collect();
        let background: Vec<f32> = (0..160 * 120).map(|i| (i % 7) as f32 / 7.0).collect();

        for erase in [false, true] {
            let mut reference = Alpha::new(160, 120, background.clone());
            let mut before = Stroke::lasso(erase);
            before.points = star.clone();
            fill_outline_as_it_was(&before, &mut reference);

            for mut lasso in
                [Stroke::lasso(erase), Stroke::soft_lasso(0.1, 0.0, erase), Stroke::soft_lasso(0.0, 1.0, erase)]
            {
                lasso.points = star.clone();
                let mut alpha = Alpha::new(160, 120, background.clone());
                lasso.draw(&mut alpha);
                assert!(
                    alpha.data.iter().zip(&reference.data).all(|(a, b)| a.to_bits() == b.to_bits()),
                    "radius {} feather {} erase {erase} differs",
                    lasso.radius,
                    lasso.feather
                );
            }
    ...
    #[test]
    fn a_soft_lasso_ramps_across_its_edge_and_covers_as_much() {
        let fill = |mut lasso: Stroke| {
            let mut alpha = Alpha::new(200, 200, vec![0.0; 200 * 200]);
            lasso.points = vec![[0.25, 0.25], [0.75, 0.25], [0.75, 0.75], [0.25, 0.75]];
            lasso.draw(&mut alpha);
            alpha.data
        };

        let (hard, soft) = (fill(Stroke::lasso(false)), fill(Stroke::soft_lasso(0.1, 0.5, false)));

        let row = &soft[100 * 200..101 * 200];
        assert_eq!(row[100], 1.0, "the middle is not solid");
        assert_eq!(row[60], 1.0, "ten cells in is not solid");
        assert_eq!(row[40], 0.0, "ten cells out is covered");
        assert_eq!(soft[5 * 200 + 5], 0.0, "it leaked into the corner");

        let edge = &row[40..=60];
        assert!(edge.windows(2).all(|pair| pair[1] >= pair[0]), "not a ramp: {edge:?}");
        let ramp = edge.iter().filter(|v| **v > 0.0 && **v < 1.0).count();
        assert!((8..=11).contains(&ramp), "the ramp is {ramp} cells for a band of 10: {edge:?}");

        assert!((row[49] + row[50] - 1.0).abs() < 0.02, "{edge:?}");

        let (was, is): (f32, f32) = (hard.iter().sum(), soft.iter().sum());
        assert!((is / was - 1.0).abs() < 0.03, "hard covers {was}, soft {is}");
    }

    #[test]
    fn a_soft_lasso_erases_with_the_same_edge() {
        let points = vec![[0.3, 0.2], [0.8, 0.4], [0.5, 0.9], [0.1, 0.6]];
        let mut added = Alpha::new(128, 96, vec![0.0; 128 * 96]);
        let mut lasso = Stroke::soft_lasso(0.08, 0.6, false);
        lasso.points = points.clone();
        lasso.draw(&mut added);

        let mut kept = Stored::new(&Alpha::new(128, 96, vec![1.0; 128 * 96]));
        let mut rubber = Stroke::soft_lasso(0.08, 0.6, true);
        rubber.points = points;
        rubber.draw(&mut kept);

        let soft = added.data.iter().filter(|v| **v > 0.0 && **v < 1.0).count();
        assert!(soft > 100, "only {soft} cells of soft edge to compare");
        for (index, (add, left)) in added.data.iter().zip(kept.values()).enumerate() {
            assert!((add + left - 1.0).abs() < 1e-4, "cell {index}: added {add}, left {left}");
        }
    }

    #[test]
    fn softness_changes_only_how_a_stroke_falls_off() {

        let (width, height) = (256usize, 129usize);
        let draw = |softness: f32| {
            let mut alpha = Alpha::new(width, height, vec![0.0; width * height]);
            let mut stroke = Stroke::new(10.0 / 256.0, softness, false);
            stroke.points = vec![[64.5 / 256.0, 0.5], [191.5 / 256.0, 0.5]];
            stroke.draw(&mut alpha);
            alpha.data
        };
        let (hard, soft) = (draw(0.0), draw(1.0));
        let distance = |index: usize| {
            let centre = [(index % width) as f32 + 0.5, (index / width) as f32 + 0.5];
            distance_to_segment(centre, [64.5, 64.5], [191.5, 64.5])
        };

        for index in 0..width * height {
            assert_eq!(hard[index] > 0.0, soft[index] > 0.0, "cell {index} is covered by one only");
            assert_eq!(hard[index], if distance(index) < 10.0 { 1.0 } else { 0.0 }, "cell {index}");
            assert!(soft[index] <= hard[index], "cell {index}");
        }
        for x in 64..=191 {
            assert_eq!((hard[64 * width + x], soft[64 * width + x]), (1.0, 1.0), "the line at {x}");
        }

        let mut profile: Vec<(f32, f32)> =
            (0..width * height).filter(|&i| soft[i] > 0.0).map(|i| (distance(i), soft[i])).collect();
        profile.sort_by(|p, q| p.0.total_cmp(&q.0));
        assert!(profile.windows(2).all(|pair| pair[1].1 <= pair[0].1), "not a falloff");
        assert!(soft.iter().filter(|v| (0.05..0.95).contains(*v)).count() > 1000);
    }

    #[test]
    #[ignore]
    fn measure_a_soft_lasso_at_full_size() {
        let (w, h) = (4096usize, 2731usize);
        let outline: Vec<[f32; 2]> = (0..600)
            .map(|i| {
                let angle = i as f32 * std::f32::consts::TAU / 600.0;
                let reach = 0.45 + 0.03 * (angle * 17.0).sin();
                [0.5 + reach * angle.cos(), 0.5 + reach * angle.sin()]
            })
            .collect();
        for (radius, feather) in [(0.0, 0.0), (0.02, 0.5), (0.1, 1.0)] {
            let mut lasso = Stroke::soft_lasso(radius, feather, false);
            lasso.points = outline.clone();
            let mut alpha = Alpha::new(w, h, vec![0.0; w * h]);
            let start = std::time::Instant::now();
            lasso.draw(&mut alpha);
            let working = start.elapsed();
            let mut kept = Stored::new(&Alpha::new(w, h, vec![0.0; w * h]));
            let start = std::time::Instant::now();
            lasso.draw(&mut kept);
            println!("radius {radius} feather {feather}: {working:?} on Alpha, {:?} on Stored", start.elapsed());
        }
    }
