...
pub fn relative_gains(from: WhiteBalance, to: WhiteBalance, from_xyz: crate::profile::Matrix3) -> [f32; 3] {
    let rgb = |balance: WhiteBalance| {
        let (x, y) = white_point_xy(balance);
        multiply_vector(&from_xyz, [x / y, 1.0, (1.0 - x - y) / y])
    };
    let (before, after) = (rgb(from), rgb(to));
    let ratio = |index: usize| match after[index].abs() < 1e-6 {
        true => 1.0,
        false => before[index] / after[index],
    };
    let gains = [ratio(0), ratio(1), ratio(2)];
    match gains[1].abs() < 1e-6 {
        true => gains,
        false => [gains[0] / gains[1], 1.0, gains[2] / gains[1]],
    }
}
...
mod tests {
    ...
    #[test]
    fn a_mask_shift_turns_the_way_the_panel_does() {
        let profile = profile();
        let matrix = crate::space::ColourSpace::Srgb.from_xyz();
        let from = WhiteBalance { temperature: 5500.0, tint: 0.0 };
        let (warmer, cooler) = (800.0, -800.0);
        for (kelvin, tint) in [(warmer, 0.0), (cooler, 0.0), (0.0, 20.0), (0.0, -20.0)] {
            let to = WhiteBalance { temperature: from.temperature + kelvin, tint };
            let (before, after) = (profile.multipliers(from), profile.multipliers(to));
            let panel = [after[0] / before[0], 1.0, after[2] / before[2]];
            let mask = relative_gains(from, to, matrix);
            match kelvin != 0.0 {
                true => assert_eq!(panel[0] > panel[2], mask[0] > mask[2], "{kelvin} K: panel {panel:?}, mask {mask:?}"),
                false => assert_eq!(
                    panel[0] + panel[2] > 2.0,
                    mask[0] + mask[2] > 2.0,
                    "tint {tint}: panel {panel:?}, mask {mask:?}"
                ),
            }
        }
    }
