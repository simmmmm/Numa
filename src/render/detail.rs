...
pub fn sharpen(
    ...
    let log = log_luminance(data, width, height);
    let base = blur(&log, scaled.round().max(1.0) as usize);
...
mod tests {
    ...
    fn detail_keeps_more_and_contrast_gives_the_texture_back() {
        ...
        let run = |detail: f32, contrast: f32| {
            ...
            denoise(&mut data, width, height, 1.0, detail, contrast, 0.0, 1.0);
            data
        };
    ...
    fn noise_reduction_smooths_grain_and_keeps_the_edge() {
        ...
        denoise(&mut cleaned, width, height, 1.0, 0.5, 0.0, 0.0, 1.0);
    ...
    fn colour_noise_reduction_keeps_the_brightness_it_found() {
        ...
        denoise(&mut data, width, height, 0.0, 0.5, 0.0, 1.0, 1.0);
    ...
    fn nothing_asked_for_is_nothing_done() {
        ...
        let mut after = before.clone();
        denoise(&mut after, width, height, 0.0, 0.5, 0.0, 0.0, 1.0);
        assert_eq!(before, after);
