...
use crate::render::matte;
use crate::render::segment::{Segmentation, MATTEABLE};
...
fn model_path() -> Option<PathBuf> {
    crate::io::model_file(&["image_classification_ppresnet50_2022jan.onnx"])
}
...
mod tests {
    ...
    fn classifiers_on_frames() {
        ...
        for path in frames.split(':') {
            ...
            let linear = crate::io::raw::decode_linear_any(&path).unwrap();
            ...
            let found = crate::render::segment::of(&frame).expect("segmentation");
            let photo = found.photo();
    ...
    fn which_frames_have_animals() {
        ...
        for path in std::fs::read_to_string(list).unwrap().lines() {
            let Ok(preview) = crate::io::raw::load_scaled(std::path::Path::new(path), 1024) else { continue };
            let Some(found) = crate::render::segment::of(&preview) else { continue };
