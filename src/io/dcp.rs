...
use crate::core::profile::{DngProfile, HsvTable, Matrix3, ValueEncoding};
...
pub fn search_paths() -> Vec<PathBuf> {
    ...
    if let Ok(exe) = std::env::current_exe() {
        if let Some(prefix) = exe.parent().and_then(|bin| bin.parent()) {
            paths.push(prefix.join("share").join(crate::io::NAME).join("profiles"));
        }
    }
...
pub fn profiles_dir() -> Option<PathBuf> {
    Some(crate::io::data_dir().join("profiles"))
}

pub fn downloaded_profiles_dir() -> PathBuf {
    crate::io::data_dir().join("rawtherapee-dcpprofiles")
}
